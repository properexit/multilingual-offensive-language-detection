...
def train_multitask(
    ...
    # Task B only exists for offensive tweets
    # so we build a quick lookup from tweet text to label
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # always compute Task A loss
            loss_a = loss_fn(logits_a, labels_a)

            # Task B only for offensive examples
            off_mask = labels_a == 1

            if off_mask.sum() > 0:
                # recover original texts for this batch
                start = batch_idx * batch_size
                batch_texts = train_a.iloc[start:start + len(labels_a)]["tweet"].tolist()
